<template>
  <div class="m-auto w-full bg-gray-100">
    ...
    <div class="relative">
      <img :src="'http://localhost:3434/images/' + images[selected].link" class="object-contain w-full h-full aspect-video m-0"/>
    ...
    <div class=" h-24 flex flex-row gap-2 justify-center flex-0 m-auto relative overflow-hidden" v-if="menu">
      ...
      <img v-for="(image, index) in images" :src="'http://localhost:3434/images/' + image.link" class="object-cover aspect-square m-0 border-primary-500 hover:border-2 hover:border-primary-300 cursor-pointer" :class="{ 'border-2' :index===selected }" @click="selected = index"/>
    </div>
